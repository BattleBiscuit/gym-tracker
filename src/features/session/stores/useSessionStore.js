    ...
    sets.value = sortedExercises.map((ex, exIdx) =>
      (ex.sets || []).map((s, setIdx) => {
        ...
        return {
          ...
          // Use last actual as placeholder only if it exists for this exact set index
          plannedReps:   last?.reps     != null ? last.reps     : (s.reps   ?? null),
          plannedWeight: last?.weight   != null ? last.weight   : (s.weight ?? null),
          actualReps:    null,
          actualWeight:  null,
          weightUnit:    s.weightUnit || 'kg',
          plannedDuration: last?.duration != null ? last.duration : (s.duration ?? null),
          plannedLevel:    last?.level    != null ? last.level    : (s.level    ?? null),
          actualDuration:  null,
          actualLevel:     null,
